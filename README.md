...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create mylists
  fly apps create mylists-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app mylists
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app mylists-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'mylists-staging' does not match app name in config file 'mylists'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name mylists-db
  fly postgres attach --app mylists mylists-db

  fly postgres create --name mylists-staging-db
  fly postgres attach --app mylists-staging mylists-staging-db
  ```
